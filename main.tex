...
\begin{column}{\colwidth}
  ...
  \begin{block}{Searching for New Physics}
    ...
    % [c] so the short, wide strategy diagram centres against the taller paragraph
    \begin{columns}[c]
    \begin{column}{0.53\columnwidth}
      The Higgs boson's \textbf{universal coupling to mass} makes it a broadly
      applicable portal to sectors beyond the Standard Model, and a natural anchor
      for anomaly detection. Our \textbf{HAXAD} (Higgs And X Anomaly Detection)
      strategy combines ML-based \textbf{feature embedding}, data-driven
      \textbf{background estimation} and \textbf{weakly supervised classification}
      to search for anomalies produced in association with a Higgs boson; a new
      inference framework now delivers \textbf{signal-agnostic} and signal-specific
      cross section limits that \textbf{match or exceed} the best limits of an
      equivalent cut-based search.
    \end{column}
    \begin{column}{0.43\columnwidth}
      \centering
      \includegraphics[width=\columnwidth]{strategy.pdf}
    \end{column}
    \end{columns}
