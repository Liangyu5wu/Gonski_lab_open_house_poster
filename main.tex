...
\begin{column}{\colwidth}
  ...
  \begin{block}{Large-Scale ML for Particle Physics}
    ...
    \begin{column}{0.52\columnwidth}
      \textbf{NEXUS} is a \emph{lightweight} foundation model --- a fully connected
      autoencoder with just \textbf{3 million parameters}, pre-trained with
      \textbf{no supervision} on \textbf{20 million} LHC collision events described
      by charged-particle tracks.

      \vspace{0.4cm}
      One pre-trained backbone then transfers to many downstream tasks --- Standard
      Model and BSM process classification, jet reconstruction, $b$-tagging, anomaly
      detection --- beating the same architecture trained from scratch while needing
      far \textbf{fewer labelled events}. The representation even carries outside
      particle physics, to \textbf{gravitational waves}, \textbf{coastal flooding}
      and \textbf{neural activity}. Being attention-free, NEXUS is far cheaper than
      a transformer of comparable scale, opening the door to \textbf{real-time and
